...
export const Thread = ({ messageId, botId, onClose }: ThreadProps) => {
    ...
    const handleSubmit = async ({ body, image }: { body: string; image: File | null }) => {
        try {
            ...
            if (body.toLowerCase().includes("@ai") || botId) {
                const contextMessage1 = `${message?.user.name}:${message?.body}`
                const contextMessages = results
                    .slice(-20)
                    .map((m) => `${m.user.name}: ${m.body}`)
                    .join('\n');
                const prompt = `You are an AI assistant inside a developer chat app. Respond concisely and use markdown when needed.
